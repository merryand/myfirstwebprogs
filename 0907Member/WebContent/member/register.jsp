...
<body>
	...
	<form action="insert" method="post" id="registerform">
		...
		<label for="pw">비밀번호</label> <input type="password" name="pw" id="pw"
			...
			type="text" name="name" id="name" required="required" /><br /> <label
			for="phone">전화번호</label> <input type="text" name="phone" id="phone"
			required="required" /><br /> <label for="addr">주소</label> <input
			type="text" size="100" name="addr" id="addr" required="required" /><br />
...
<script>
	...
	document.getElementById("registerform").addEventListener("submit",
			function(e) {
				...
				if (emailcheck == false) {
					alert("이메일 중복체크를 하세요!!!")
					emailmsg.innerHTML = "이메일 중복체크를 다시 하세요"
