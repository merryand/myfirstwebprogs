...
public class FileController extends HttpServlet {
	...
	public FileController() {
		super();
		fileService = FileUploadServiceImpl.sharedInstance();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// 공통된 요청 주소를 서버 경로 부분을 제외한 요청 추출하기
		// 전체 요청 경로 가져오기
		String requestURI = request.getRequestURI();
		// contextPath : 서버 경로
		String contextPath = request.getContextPath();
		// requestURI에서 contextPath와 /제거하기
		...
		case "fileupload.file":
			// 단순 페이지 이동이나 서버에서 데이터를 조회한 후 출력할 때는 포워딩을 주로 이용합니다.
		...
		case "upload.file":
			...
			/*
			 * // 처리 결과를 저장 request.setAttribute("map", map); // 처라 결과를 출력할 페이지로 이동 - 포워딩 -
			 * 아래 경로 안나옴 - 새로고침하면 또 처리 해버림. dispatcher =
			 * request.getRequestDispatcher("views/result.jsp"); dispatcher.forward(request,
			 * response);
			 */
			...
			// 리다이렉팅을 할 때는 결과 페이지 경로를 적는 것이 아니라
